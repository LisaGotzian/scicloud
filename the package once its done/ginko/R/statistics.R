...
################################
# 28.09. Added require(SnowballC) und require(ggrepel)
#         added a path with "./PDFs" so we can set the working directory as the entire folder
#         at around 724: added a dendrogram plot with a corresponding argument in the big function
#         as well as the option to either break the lines or truncate them.
#
# 5.10. DOIs that couldn't be retrieved (NA) are not being thrown out (Added incomparables = NA to line 84,
#        deleted the exact same line in 254), Julius & Stefan encountered the problem of having 150 papers
#        thrown out of their analysis due to this. For this,....
#
#        - changed line 252 to account if nchar()<100 not is.na -> throws out empty full texts like PDFs that
#         are an image.
#
#        - I adjusted line 456 to accomodate for the change in number of rows. length(wordMatrix)

#         renamed a number of variables:
#           temp5 becomes indSpeciesValues
#           hvwtest becomes combIndSpeciesValues
#           hvwtestp and hvwtestp2 becomes signIndSpeciesValues
#           hvwtestp2InclSubsetRow in signIndSpeciesValuesInclSubsetRow
#           highestIntValPerCluster in highestIndValPerCluster (changed the t as not an interval but an indicator is meant)
#
# 6.10.   Representative analysis based on the number of significant papers per Cluster in 782
#
# 7.10. Replaced all output messages that use print() with cat(). Cat() doesn't print [1] but requires
#         \n at the end of the line for a line break
#        Improved the functions' descriptions
#
# 26.10. starting in 808, I added a weighted percentageOfSignWordsInPaper based on the indicator
#         species values.
#         added the new arguments "dendrogram = TRUE, dendroLabels = "truncated" " to the wrapper function
#
# 16.11. added row.names() around 546 to processedData[[1]] so the adjacency
#         matrix has rownames
#
# 18.11. added a new function: "calculateNetwork" to do computations based on network theory.
#         see the help for more input.
#
# 19.11. fulfilled Hanna's long-lasting dream and added a way to exactly specify the
#         words ginko uses (you can define a moritz file)
#
# 26.11. changed the Felix print commands back to normal
#
# 27.11. FEATURE: added saveRDS() in every function, added an argument saveToWd to switch this off.
#         added ordinationFunction=FALSE to every function to change details
#         when working with the wrapper function ordinationCluster(). Examples:
#         - readline() to pause after the Dendrogram
#         - all messages after saveRDS() are altered if ordinationFunction==TRUE
#
#         NEW: added a new function named inspectGinko(). It replaces some parts in the using_ginko_explained.R
#         For this, I added the _original_ wordlist as a [[3]] output to processedMetaMatrix
#         and a [[4]] output to modeledData. This way, the nr of original words can be
#         retrieved (list is even before ignoreWords).
#
#         USABILITY: named many list outputs, in particular:
#         - processedMetaMatrix[[1]] to processedMetaMatrix$BinaryWordList,
#           [[2]] to $MetaMatrix and [[3]] to $numberOfWords
#         - modeledData[[1]] to modeledData$IndVal, [[2]] to $MetaMatrix,
#           [[3]] to $RepresentativePapers and [[4]] to $numberOfWords
#         - networkMatrix[[1]] to networkMatrix$LocalMeasures,
#           [[2]] to $ReducedLocalMeasures, [[3]] to $ReducedIncidenceMatrix,
#           [[4]] to $GlobalMeasures
#
#         - unreadable indexing such as [,18] to [,"FileName"]
#
#         NETWORK:
#         - Originally, the dataframe sorted both papers and words
#           for centrality measures, now it throws out the papers in the analysis
#           if they have ".pdf" in their name. (Words don't have ".pdf".)
#         - added the network approach to the wrapper ordinationCluster(). It can
#           be called using either: method = c("hclust", "network", "both").
#
#         processMetaMatrix: Stefan & Julius adjusted gsub() to keep everything that is letters
#
#         Minor changes: changed language for tm from "english" to "SMART" in both
#         processMetaDataMatrix() and ordinationCluster().
#         The wrapper function ordinationCluster() now returns both modeledData and modeledNetwork
#         Changed all metaDataMatrix objects to metaMatrix (prone to errors, @Lisa check!)
#         Entered a warning message in createOrdinationPlot() to tell the user to get an API key.
#         Adjusted mostImportantPapersPerCluster() also for very small datasets using
#         nrow(newSubset). If the # of papers per cluster i is smaller than the desired
#         paperPerCluster = 5, this cluster won't be used.
#
# 8.1.19 - added a readline() if ordinationCluster() has method = "both" to not have such a
#         clutter in the output
#         - introduced longMessages as an argument, the normal output is then super-short.
#
# 14.1.  - save each paper into one new folder in calculateModels
#
# 14.2.  - added a message for when the package is loaded.
#         - added a @family ginko functions tag and lots of @seealso tags, mostly to the
#           proceeding functions
#         - removed all require() and library()
#         - adjusted pkgname::function() for all functions
...
.onAttach <- function(libname, pkgname) {
  packageStartupMessage("Welcome to ginko. Have a folder 'PDFs' within your working directory that contains all scientific papers you intend to use. Run ?ginko to get started.")
}
...
#' }
#' @export
createTextMatrixFromPDF <- function(filepath = "./PDFs/", saveToWd = TRUE){



	PDFFiles <- paste0(filepath,list.files(path = filepath))
	PDFcontent <- matrix(NA, nrow = length(PDFFiles), ncol = 20)
	colnames(PDFcontent) <- c("Title", "Year", "Month", "Day", "Authors",
	                          "Journal", "Volume", "Issue", "Pages", "CitedBy",
	                          "CitationPerYear", "DOI", "Scopus-ID", "Publisher",
	                          "Affiliation", "Affiliation-City",
	                          "Affiliation-Country", "FileName", "Abstract", "FullText")
	...
	for (i in c(start:end)){
	    ...
	    intermediateResultText <- tryCatch({
			reader <- tm::readPDF(control = list(text = "-raw")) # using the default
			suppressWarnings(reader(elem = list(uri = intermediateResultFileName),
			                        language = "en", id = "id1"))},
			error=function(cond) {
	            message(paste("PDF File caused an error while retrieving the full text",
	    ...
	    intermediateResultText <- paste(intermediateResultText, collapse=" ")  #takes the vector and pastes it into a single element, seperated by a " "

	    # get rid of the path for the filename
		PDFcontent[i,"FileName"] <- if (length(intermediateResultFileName)>0){
		  gsub("./PDFs/", " ", intermediateResultFileName)
		}else{
		    NA
		}

		PDFcontent[i,"FullText"] <- if (length(intermediateResultText)>0){intermediateResultText}else{NA}

	#Progress Bar
	utils::setTxtProgressBar(pb, i)
	}

	PDFcontent <- cbind(PDFcontent, "ID"=1:nrow(PDFcontent)) #assiging a unique id to avoid collisions along the way

	DOIpattern <- '\\b(10[.][0-9]{4,}(?:[.][0-9]+)*/(?:(?!["&\'<>])[[:graph:]])+)\\b'
	DOInumbers <- stringr::str_extract(PDFcontent[,"FullText"], DOIpattern)
...
ordinationCluster <- function(metaMatrix,
  ...
  if (sum(c(method == "hclust",
      method == "network",
      method == "both")) != 1){
    cat("Please use one of the possible inputs for 'method': 'hclust', 'network' or 'both'. Default is 'hclust'.")
  }
  ...
  ordinationFunction <-  TRUE
	...
	processedMetaMatrix <- processMetaDataMatrix(metaMatrix, language = language, stemWords = stemWords, ignoreWords = ignoreWords,
	                                             ordinationFunction = ordinationFunction, saveToWd = saveToWd, longMessages = longMessages)

	processedMetaMatrix$MetaMatrix <- getScopusMetaData(processedMetaMatrix$MetaMatrix, myAPIKey, ordinationFunction = TRUE)

	if (any(c(method == "hclust"), (method == "both"))){

  	modeledData <- calculateModels(processedMetaMatrix, numberOfClusters = numberOfClusters, minWordsPerCluster = minWordsPerCluster,
	                               maxWordsPerCluster = maxWordsPerCluster, p = p, dendrogram = dendrogram,
	                               dendroLabels = dendroLabels, ordinationFunction = ordinationFunction, saveToWd = saveToWd,
	                               longMessages = longMessages)
  	createOrdinationPlot(modeledData, exactPosition = exactPosition, ordinationFunction = ordinationFunction)
	...
	if (any(c(method == "network"), (method == "both"))){
	  modeledNetwork <- calculateNetwork(processedMetaMatrix, saveToWd = saveToWd, ordinationFunction = ordinationFunction, longMessages = longMessages)
	}

	if(method == "hclust") return(modeledData)
	if(method == "network") return(modeledNetwork)
	if(method == "both") return(c(modeledData, modeledNetwork))
...
processMetaDataMatrix <- function(metaMatrix,
  ...
  if(all(c(useMoritz == TRUE, !moritzFile %in% list.files()))){
    cat("If you'd like to use a file to specify the words you'd like to keep, please have it in your working directory.")
  }

  # is it a csv or csv2?
  if(useMoritz == TRUE){
    moritz <- utils::read.csv(file = moritzFile)
    if(grepl(";", moritz[1,])){ #if a semicolon is in there...
  ...
  if (longMessages == TRUE){
    Sys.sleep(1)
	  ...
	  pb <- utils::txtProgressBar(min = 1, max = nrow(metaMatrix), style = 3)
  }
	...
	for (j in 1:nrow(metaMatrix)) {
	  ...
	  if (longMessages == TRUE){utils::setTxtProgressBar(pb, j)}
	...
	if(length(textlessIDs) != 0){
		...
		for(k in 1:length(textlessIDs)){
			intermediateTextlessIDs <- which(grepl(textlessIDs[k], metaMatrix[,"ID"]))
			rowsOfTextlessIDs <- c(rowsOfTextlessIDs, intermediateTextlessIDs)
		}

		metaMatrix <- metaMatrix[-rowsOfTextlessIDs, ]
	}
	if (longMessages == TRUE){close(pb)}
	...
	for (i in c(1:nrow(metaMatrix))){
		...
		if((!is.na(metaMatrix[i,"FullText"]))&(length(metaMatrix[i,"FullText"]) == 1)){
			doc <- metaMatrix[i,"FullText"]
		}else if((!is.na(metaMatrix[i,"Abstract"]))&(length(metaMatrix[i,"Abstract"]) == 1)){
		...
		textBody <- paste(textBody, collapse=" ")  #takes the vector and pastes it into a single element, seperated by a " "
		textBody <- gsub("[\r\n]+", " ", textBody)
		# textBody <- gsub("- ", "-", textBody)
		textBody <- gsub("[^ -abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-]+", " ",textBody)  ############## Added 10.10.2018: Replace every character with an empty space except for those in the squared brackets

		textBody <- gsub("- ", "", textBody)  # undo hyphenation
		textBody <- gsub('\f', " ", textBody) # seems to be definition for page brake or something eg brake from footer/header to text
		#textBody <- gsub('\', " ", textBody)
		#punctuations
		textBody <- gsub("[[:punct:]]", " ", textBody)
		textBody <- gsub("[0-9]", " ", textBody)
		textBody <- gsub("", "", textBody)    ############### Added 10.10.2018
		textBody <- gsub("    ", " ", textBody)
		textBody <- gsub("   ", " ", textBody)
		textBody <- gsub("  ", " ", textBody)

		textBody <- tolower(textBody) #at this point the data is a vector with a single element, containing the fulltext of the source
		...
		words <- strsplit(textBody," ") #creating a vector with every singe word being one element
		wordTableList[[i]] <- data.frame(table(words)) #creates a dataframe, that has all the words in col1 and their frequency in col2


		names(wordTableList)[i] <- as.character(metaMatrix[i,"ID"]) #current workaround to avoid strange truncation behaviour, gives the list entry the actual name of the paper
		...
		#Progress Bar
		if (longMessages == TRUE){utils::setTxtProgressBar(pb, i)}
		})
	}
	if (longMessages == TRUE){close(pb)}
	...
	if (longMessages == TRUE){
	  ...
	  pb <- utils::txtProgressBar(min = 1, max = length(wordTableList), style = 3)
  }
  ...
  # The following section adds all words that are supposed to be included like smaller than 16 or
	...
	numberOfWords <- includes #How many words where there originally? For later use.

	# Apply moritz
	if (useMoritz == TRUE){
	  # keep only the words denoted with "1"
	  if (ncol(moritz) == 2) moritzwords <- moritz[moritz[,2]==1,1]

	  moritzwords <- as.character(moritzwords) # for convenience

	  wordssub <- includes[includes %in% moritzwords]

	}else{ # if useMoritz = FALSE


  	wordssub <- includes[which(nchar(as.character(includes))<16)] #only keep entries with less than 16 letters
  	wordssub <- wordssub[which(nchar(as.character(wordssub))>2)] #only keep entries with more than 2 letters
  	...
  	for (i in c(1:nrow(metaMatrix))){
  		...
  		if(!is.na(metaMatrix[i,"Authors"])){
  			...
  			authors <- as.character(unlist(strsplit(metaMatrix[i,"Authors"], ", ")))
  			allAuthors <- c(allAuthors, authors)
  		}
  	}
  	allAuthors <- sort(unique(tolower(allAuthors)))

  	wordssub <- wordssub[! wordssub %in% allAuthors] #filtering the authors here and not through the "ignoreWords" variable, because of performance reasons
  	...
  	for (i in c(1:nrow(metaMatrix))){
  		...
  		if(!is.na(metaMatrix[i,"Publisher"])){
  			...
  			publisher <- as.character(unlist(strsplit(metaMatrix[i,"Publisher"], ", ")))
  	...
  	wordssub <- wordssub[! wordssub %in% allPublishers] #filtering the publishers here and not through the "ignoreWords" variable, because of performance reasons
  	...
  	wordssub <- wordssub[! wordssub %in% ignoreWords] #filtering the ignored Words here and not through the "ignoreWords" variable, because of performance reasons
	...
	colnames(wordMatrix)<-"words" #at this point, wordMatrix is one single dataframe with all the words in one column called words
  #PapersNotReadIn <- NULL
	...
	for (j in c(1:length(wordTableList))){
	  ...
	  # the line above throws out PDFs that couldn't be read like PDFs that are an image
		...
		datasub1 <- datasub[datasub$words %in% wordssub,] #this line removes the words, that have been removed while creating the "wordsub" variable from the individual list
		wordMatrix <- merge(wordMatrix, datasub1, by="words",all.x=T) # generates a dataframe with a row per word, and a col per paper including the amount of appearances
		if (longMessages == TRUE){utils::setTxtProgressBar(pb, j)}
	  #} #else {
	...
	wordMatrix2 <- wordMatrix[,2:(length(wordMatrix))] #the first col are the words, followed by the individual papers
	rownames(wordMatrix2) <- wordssub #correcting the shift in numbers, and assigning the words to the corresponding rowname
	...
	#dim(wordMatrix2)
	for (k in 1:length(wordMatrix2)) wordMatrix2[is.na(wordMatrix2[,k]),k] <- 0 #change all NA in dataframe to 0
	colnames(wordMatrix2) <- names(wordTableList)[c(1:length(wordTableList))] #set title as colname
	...
	wordMatrix3 <- wordMatrix2[apply(ifelse(wordMatrix2>0,1,0),1,sum)>(length(wordTableList)/100*5),] #keep only words that appear in more than 5% of the papers

	#rownames(wordMatrix3);dim(wordMatrix3)
	wordMatrix3Binary <- ifelse(wordMatrix3>0,1,0) #transform wordMatrix3 to binary matrix
	...
	processedData <- list()
	processedData[[1]] <- wordMatrix3BinaryTransposed[apply(wordMatrix3BinaryTransposed,1,sum)>=1,] #excludes all rows, with a sum of 0. (papers with no matching words)
	row.names(processedData[[1]]) <- metaMatrix[,"FileName"] #rename the rows to use the adjacency matrix



	excludedPapers <- rownames(wordMatrix3BinaryTransposed[apply(wordMatrix3BinaryTransposed,1,sum)<1,])
	...
	if(!is.null(excludedPapers)){
		...
		for(l in 1:length(excludedPapers)){
			intermediateExcludedRows <- which(grepl(excludedPapers[l], metaMatrix[,"ID"]))
...
calculateModels <- function(processedData,
  ...
  if (longMessages == TRUE){
	...
	model <- vegan::decorana(processedData$BinaryWordList, iweigh=0) #all row sums must be >0 in the community matrix
	...
	disthclust <- stats::dist(processedData$BinaryWordList, method="binary")
	...
	if(is.na(numberOfClusters)){
		...
		while(numberOfSignificantIndicators < numberOfSignificantIndicatorsNew){
			...
			cutmodel <- stats::cutree(modelclust, k=numberOfClusters) #assigns a cluster number to every paper
			#table(cutmodel)
			indSpeciesValues <- labdsv::indval(processedData$BinaryWordList, cutmodel, numitr=1000)
			if (longMessages == TRUE){cat(paste0("- Number of clusters: ", numberOfClusters, "; significant words (p < ", p, "): ", length(indSpeciesValues$pval[indSpeciesValues$pval<=p]),"\n"))	}


			numberOfSignificantIndicatorsNew <- length(indSpeciesValues$pval[indSpeciesValues$pval<=p])
  ...
  # Combines all relevant values for the indicator species analysis (including axis positions)
	combIndSpeciesValues <- cbind(round(indSpeciesValues$indval, digits=2), indSpeciesValues$pval,
	                 names(indSpeciesValues$pval), axisPositions)

	# Only takes the p values that are smaller than the given p value passed as an argument (0.05 per default)
	signIndSpeciesValues <- combIndSpeciesValues[combIndSpeciesValues["indSpeciesValues$pval"] <= p,]
	...
	for (i in 1:numberOfClusters) {
		...
		highestRankedWord <- signIndSpeciesValues[order(signIndSpeciesValues[,i],decreasing=T)[c(1)],]
		highestIndValPerCluster <- c(highestIndValPerCluster, highestRankedWord[,i])
	}
	...
	highValues <- (signIndSpeciesValues[,1]>min(highestIndValPerCluster))
	...
	signIndSpeciesValuesInclSubsetRow <- signIndSpeciesValues[order(signIndSpeciesValues[,1],decreasing=T)
	                                                          [c(1: numberOfWords)],]
	signIndSpeciesValuesInclSubsetRow$subset <- paste0("Cluster ", rep.int(1, numberOfWords))



	for (i in 2:numberOfClusters) {

		highValues <- (signIndSpeciesValues[,i]>min(highestIndValPerCluster))
		highValues <- length(highValues[highValues==TRUE])
		...
		subset <- signIndSpeciesValues[order(signIndSpeciesValues[,i],decreasing=T)[c(1: numberOfWords)],]
	...
	if (dendrogram == TRUE){
	  if (dendroLabels == "break"){ # this breaks the file names
    	wordwrap<-function(x,len) paste(strwrap(x,width=len),collapse="\n")
    	graphics::plot(modelclust, cex = 0.6, hang = -1, main = "Word cluster dendrogram of papers",
    	     labels = sapply(processedData$MetaMatrix[,"FileName"],wordwrap,len=15), padj = 1)
    	#18th column is the filename, strwrap() splits the labels
	  }
	  if (dendroLabels == "truncated"){ # this truncates the labels to 18 characters, followed by "..."
    	longLabels <- processedData$MetaMatrix[,"FileName"]
    	shortenedLabels <- NULL
    	for (i in 1:length(longLabels)){
    	  shortenedLabels[i]<-ifelse(nchar(longLabels[i])>20,  # shorten labels from 1:18
    	                             paste(paste(unlist(strsplit(longLabels[i],""))[1:18],sep="",collapse=""),
    	                                   "...",sep="",collapse=""),longLabels[i])
    	}
      graphics::plot(modelclust, cex = 0.6,
           hang = -1,
	...
	modeledData[[2]] <- cbind(processedData$MetaMatrix, "Cluster"=Cluster)
	...
	ClusterContent <- signIndSpeciesValues[,"names(indSpeciesValues$pval)"]
	...
	representativePapers <- as.data.frame(processedData$BinaryWordList[,ClusterContent[,1]])
	rownames(representativePapers) <- processedData$MetaMatrix[,"FileName"] # take the filenames as row names
	...
	for (i in 1:nrow(representativePapers)){
	  ...
	  representativePapers[i,] <- as.numeric(representativePapers[i,]) * #take the 0/1 if the word exists
	...
	representativePapersEasyToOpen <- cbind(representativePapers$percentageOfSignWordsInPaper, Cluster)
	representativePapersEasyToOpen <- as.data.frame(representativePapersEasyToOpen)
	rownames(representativePapersEasyToOpen) <- trimws(processedData$MetaMatrix[,"FileName"]) # take the filenames as row names
	colnames(representativePapersEasyToOpen) <- c("percentageOfSignWordsInPaper", "Cluster")

	modeledData[[3]] <- representativePapersEasyToOpen

	modeledData[[4]] <- processedData$numberOfWords
	names(modeledData) <- c("IndVal", "MetaMatrix", "RepresentativePapers", "numberOfWords")
	...
	for(i in 1:nlevels(as.factor(cutmodel))){
	  ...
	  file.copy(paste0("PDFs/", rownames(representativePapersEasyToOpen[representativePapersEasyToOpen[,2]==i,])),
	            to=paste("PdfsPerCluster/",i),copy.mode=T)
	}
...
createOrdinationPlot <- function(modeledData,
	...
	if(exactPosition == TRUE){
	  # This is the cluster cloud plot with labels to exact positions
		...
		ggplot2::geom_point(ggplot2::aes(x = modeledData[[1]]$DCA1, y = modeledData[[1]]$DCA2), size = 4, color = 'grey') +
		ggrepel::geom_label_repel(
	    	ggplot2::aes(modeledData[[1]]$DCA1, modeledData[[1]]$DCA2, fill = factor(subset), label = modeledData[[1]][,"names(indSpeciesValues$pval)"]),
	...
	naFreeData1 <- subset(modeledData$MetaMatrix, !is.na(modeledData$MetaMatrix[,"Year"]))
	...
	for (i in 1:nrow(naFreeData)) {
		...
		intermediateSum <- sum(as.numeric(naFreeData$CitedBy[naFreeData[,"Year"] == naFreeData[i,"Year"]]))
		percentage <- intermediateCitedBy/intermediateSum*100
		citePercent <- c(citePercent, percentage)
	}
	if (nrow(naFreeData) == 0){
	  cat(paste0("Please use your API key to obtain the metadata of your papers from scopus. Then, I'll be able to generate plots of citations and years.\n"))
	}
	...
	for (i in 1:nrow(naFreeData)) {
		...
		intermediateSum <- sum(as.numeric(naFreeData$Cluster[naFreeData[,"Year"] == naFreeData[i,"Year"]]))
	...
	naFreeData$ClusterString <- apply(naFreeData["Cluster"], 1, function(x) paste0("Cluster ", x))
	 ...
	 #    ggplot2::geom_area()
		...
		citationsStackedBarPlot <- ggplot2::ggplot(naFreeData,
			ggplot2::aes(
				x= naFreeData$Year,
				y= as.numeric(naFreeData$CitedBy),
		...
		citationsStackedBarPlotPercent <- ggplot2::ggplot(naFreeData,
			ggplot2::aes(
				x= naFreeData$Year,
				y= as.numeric(naFreeData$citePercent),
				fill = naFreeData$ClusterString
		...
		paperPerClusterPerYearStackedBarPlot <- ggplot2::ggplot(naFreeData,
			ggplot2::aes(
				x= naFreeData$Year,
		...
		paperPerClusterPerYearStackedBarPlotPercent <- ggplot2::ggplot(naFreeData,
			ggplot2::aes(
				x= naFreeData$Year,
...
mostImportantPaperPerCluster <- function(modeledData){
	...
	numberOfClusters <- nrow(as.data.frame(table(modeledData$MetaMatrix[,"Cluster"]))) #get the number of clusters from the provided data

	for(i in 1:numberOfClusters){
	  paperPerCluster = 5

		newSubset <- subset(modeledData$MetaMatrix, modeledData$MetaMatrix[,"Cluster"]==i)

		newSubset[,"CitedBy"] <- as.numeric(newSubset[,"CitedBy"])
		newSubset[,"CitationPerYear"] <- as.numeric(newSubset[,"CitationPerYear"])

		if (nrow(newSubset) < paperPerCluster){
		  cat(paste0("*** Cluster ",i," only has ",nrow(newSubset)," papers. Please consider a bigger dataset for more reliable results. ***\n"))
		...
		newSubset2 <- newSubset[order(as.numeric(newSubset[,"CitedBy"]),decreasing=T),]
		orderedSubset <- newSubset2[order(as.numeric(newSubset2[,"CitationPerYear"]),decreasing=T)[c(1: paperPerCluster)],]
		...
		CitationPerYear <- as.vector(unlist(orderedSubset[,"CitationPerYear"]))
		...
		for (j in 1:paperPerCluster){
			...
			Authors <- as.vector(unlist(strsplit(orderedSubset[j,"Authors"], ", ")))
			...
			paperName <- paste0(Authors, ' (', Year, '). "', Title, '" In: ', Journal, " ", if(!is.na(Volume)){Volume}else{""}, if(!is.na(Issue)){"."}else{""}, if(!is.na(Issue)){Issue}else{""}, if(!is.na(Pages)){", pp."}else{""}, if(!is.na(Pages)){Pages}else{""}, if(!is.na(DOI)){". DOI: "}else{""}, if(!is.na(DOI)){DOI}else{""})

			paperNames <- c(paperNames, paperName)

		}

		printWithBlankLines <- function(x){cat(x, sep="\n\n")}

		writeLines(c("", "",
			paste0("Most influencial papers in cluster ", i, ":"),
			paste0("Citations per Year: ", paste(CitationPerYear, collapse=", ")),
...
calculateNetwork <- function(processedMetaMatrix,
  ...
  if (any(c(is.null(processedMetaMatrix[[1]]),
  ...
  if (sum(c(sortby == "Eigenvector",
            sortby == "Degree",
            sortby == "Closeness",
            sortby == "Betweenness")) != 1){
    cat("Please use one of the possible inputs for 'sortby': 'Eigenvector', 'Degree', 'Closeness', 'Betweenness'. Default is 'Eigenvector'.")
  }
  ...
  snaIncidence <- igraph::graph.incidence(sna.transposed, directed = FALSE)
  snaEdgelist <- igraph::as_edgelist(snaIncidence)


  #snaTnet <- as.tnet(snaEdgelist, type = "binary two-mode tnet")
  snaTnet <- bipartite::web2edges(sna.transposed, weight.column = FALSE, return = TRUE)
  ...
  pb <- utils::txtProgressBar(min = 1, max = 5, style = 3)
  }

  # Degree
  nodeDegree <- igraph::degree(snaIncidence)
  if (longMessages == TRUE){utils::setTxtProgressBar(pb,1)}
  ...
  if (longMessages == TRUE){utils::setTxtProgressBar(pb,2)}

  # Closeness
  nodeCloseness <- igraph::closeness(snaIncidence)
  if (longMessages == TRUE){utils::setTxtProgressBar(pb,3)}

  # Eigenvector
  nodeEigenvector <- igraph::eigen_centrality(snaIncidence)$vector
  if (longMessages == TRUE){utils::setTxtProgressBar(pb,4)}


  networkMatrix <- list()
  networkMatrix[[1]] <- cbind(nodeDegree, nodeBetweenness, nodeCloseness, nodeEigenvector) # [[1]] will be the papers
  if (longMessages == TRUE){utils::setTxtProgressBar(pb,5)}
  ...
  pb <- utils::txtProgressBar(min = 1, max = 6, style = 3)
  ...
  clusterWalktrap <- igraph::membership(igraph::cluster_walktrap(snaIncidence, steps = 10))


  clusterGreedy <- igraph::membership(igraph::fastgreedy.community(snaIncidence))
  if (longMessages == TRUE){utils::setTxtProgressBar(pb, 2)}

  #clusterEdgeBetweenness <- cluster_edge_betweenness(snaIncidence) #took forever, abandoned
  clusterInfomap <- igraph::membership(igraph::cluster_infomap(snaIncidence))
  if (longMessages == TRUE){utils::setTxtProgressBar(pb, 3)}

  networkMatrix[[1]] <- cbind(networkMatrix[[1]], clusterWalktrap, clusterGreedy, clusterInfomap)
  ...
  networkMatrix[[2]] <- networkMatrix[[1]][sort.list(networkMatrix[[1]][,sortbyName], decreasing = TRUE),]

  # a parameter used later as well that determines the number of rows to be kept.

  namefirstcolumn <- rownames(networkMatrix[[2]]) #### Rookie Version: creates an accessible duplicate of the rownames
  networkMatrix[[2]] <- cbind(namefirstcolumn, networkMatrix[[2]])
  test <- stringr::str_detect(networkMatrix[[2]][,"namefirstcolumn"], ".pdf") #### Deletes all the papers based on .pdf
  networkMatrix[[2]] <- cbind(test, networkMatrix[[2]])
  networkMatrix[[2]] <- networkMatrix[[2]][networkMatrix[[2]][,"test"] =="FALSE",] #### only keeps the rows that do not contain .pdf
  networkMatrix[[2]] <- networkMatrix[[2]] [,3:9] #### Rookie way of Matrix cleaning (deleting the test and namefirstcolumn thingies)
  if (longMessages == TRUE){utils::setTxtProgressBar(pb, 4)}
  nrowMatrix <- round(
    (nrow(networkMatrix[[2]])*keep))
  ...
  networkMatrix[[3]] <- sna[, testvector==TRUE] ### this is the reduced incidence matrix based on keep
  if (longMessages == TRUE){utils::setTxtProgressBar(pb, 5)}

  utils::write.csv(file = "reduced_incidence_matrix.csv", x = networkMatrix[[3]])
  if (longMessages == TRUE){utils::setTxtProgressBar(pb, 6)
    close(pb)}

  # then let it run in gephi, do all the algorithms that didn't work

  ##### Global Measures
  if (longMessages == TRUE){Sys.sleep(1)
  cat("Calculating global measures...\n")
  Sys.sleep(1)

  pb <- utils::txtProgressBar(min = 1, max = 2, style = 3)}

  meanDistance <- igraph::mean_distance(snaIncidence, directed = FALSE)
  if (longMessages == TRUE){utils::setTxtProgressBar(pb, 1)}
  ...
  cat("Find the .csv-file reduced based on centrality in your working directory for further use.\n")

  if (saveToWd == TRUE){
    modeledNetworkFile <- paste0("modeledNetwork", format(Sys.time(), "%Y_%m_%d_%H_%M_%S") )
    saveRDS(networkMatrix, file = modeledNetworkFile)

    if (ordinationFunction == FALSE){
      cat(paste0("\nThe complete modeled Network Data is now in your global environment. It is also saved as a file in your working directory. If you work with the same data again, you can skip this step in future analysis by reading in the file:\nmodeledNetwork <- readRDS(file= '", modeledNetworkFile, "')\n\n"))
    } else {
      cat(paste0("Network Data saved. You can read it in using:\nmodeledNetwork <- readRDS(file= '", modeledNetworkFile, "')\n###################################################################################################\n\n"))

    }
  }
  names(networkMatrix) <- c("LocalMeasures", "ReducedLocalMeasures", "ReducedIncidenceMatrix", "GlobalMeasures")
...
inspectGinko <- function(modeledData = modeledData){
  ...
  paperCluster <- cbind(modeledData$MetaMatrix[,"Cluster"], modeledData$MetaMatrix[,"FileName"])
  colnames(paperCluster) <- c("Cluster", "FileName")

  OriginalPapers <- modeledData$MetaMatrix[,"FileName"]
  excludedPapers <- setdiff(trimws(OriginalPapers), trimws(paperCluster[,2]))

  cat(paste0("Summary of the ginko analysis\n\nTotal papers: ", length(OriginalPapers), ", processed papers: ", length(paperCluster[,2]),
      ", excluded papers: ", length(excludedPapers), ",\nTotal words: ", length(modeledData$numberOfWords),
      ", words that are in 5% of all papers that have been used for the analysis: ", length(levels(modeledData[[1]]$`names(indSpeciesValues$pval)`)), # that is a very weird way to extract them.
      "\n\nPapers per Cluster:\n"
      ))
  ...
  cat("- Words-Cluster-Table: Words do not belong to one single cluster. An indicator species analysis shows how representative each word is for each cluster. Use View(GinkoSpecs$IndVal) to view the results of the indicator species analysis.\n")

  # So which papers have been excluded in step 2? (step 1 will come)
  GinkoSpecs[[3]] <- excludedPapers

  cat("- excluded Papers: use View(GinkoSpecs$excludedPapers) to see which papers have been excluded, possibly because of a PDF error.\n")

  GinkoSpecs[[4]] <- modeledData$RepresentativePapers
  cat("- most representative Papers: use View(GinkoSpecs$representativePapers) to see which papers are the most representative ones, weighted with the indicator species values of the words in the paper.\n")

  GinkoSpecs[[5]] <- modeledData$MetaMatrix[,- which(colnames(modeledData$MetaMatrix) == "FullText")]
  # excluding with - doesn't work with ""
  cat("- Matrix with metadata of the papers: use View(GinkoSpecs$MetaMatrix) to view the original MetaMatrix (without full texts, so it's safe to open).")

  names(GinkoSpecs) <- c("paperCluster", "IndVal", "excludedPapers", "representativePapers", "MetaMatrix")
